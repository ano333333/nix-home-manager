...
local image_layer_enter_pressed =
  image_layer("nekotyan/image-enter-pressed.png")
local image_layer_headpat_center =
  image_layer("nekotyan/image-headpat-center.png")
local image_layer_headpat_left = image_layer("nekotyan/image-headpat-left.png")
local image_layer_headpat_right =
  image_layer("nekotyan/image-headpat-right.png")
local image_layer_headpat_finished_1 =
  image_layer("nekotyan/image-headpat-finished-1.png")
local image_layer_headpat_finished_2 =
  image_layer("nekotyan/image-headpat-finished-2.png")
  ...
  if window:is_focused() then
    if headpat_finished == 1 then
      window:set_config_overrides({
        background = { background_layer, image_layer_headpat_finished_1 },
      })
    elseif headpat_finished == 2 then
      window:set_config_overrides({
        background = { background_layer, image_layer_headpat_finished_2 },
      })
    elseif csa_count > 0 then
      ...
      if frame % 2 == 0 then
        window:set_config_overrides({
          background = { background_layer, image_layer_headpat_center },
        })
      elseif frame % 4 == 1 then
        window:set_config_overrides({
          background = { background_layer, image_layer_headpat_left },
        })
      else
        window:set_config_overrides({
          background = { background_layer, image_layer_headpat_right },
        })
      end
    elseif command_enter_pressed then
      window:set_config_overrides({
        background = { background_layer, image_layer_enter_pressed },
      })
    elseif blinking then
      ...
      window:set_config_overrides({
        background = { background_layer, image_layer_default },
      })
    end
  else
...
wezterm.on(
  "nekotyan:headpat-call-after",
  function(window, pane, csa_prev_pressed_time)
